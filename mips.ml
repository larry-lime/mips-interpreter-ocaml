...
let test_reg rf expected =
  List.map
    (fun (r, v) ->
      let v' = rf_lookup (reg2ind r) rf in
      if v' = v || v' = rev_endianess v then ()
      else
        print_string
          (format_string "failed (expected,got): " Bright Red
...
let test_pc pc1 pc2 (f, v) =
  match f with
  | 1 ->
      if Int32.add pc1 v = pc2 then
        print_string (format_string "PC passed\n" Bright Green)
      else print_string (format_string "PC failed\n" Bright Red)
  | _ ->
      if pc2 = v then print_string (format_string "PC passed\n" Bright Green)
      else print_string (format_string "PC failed\n" Bright Red)
...
let _ =
  if Array.length Sys.argv = 1 then test_interpreter () else parse_and_run ()
