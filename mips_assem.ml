...
let rec ones n : int32 =
  if n = 0 then Int32.zero
  else Int32.add (Int32.shift_left Int32.one (n - 1)) (ones (n - 1))

(*rewrite all pseudo instructions*)
let rec rem_pseudo (prog : program) : program =
  List.fold_right
    (fun (i : inst) (p : program) ->
      match i with
      | Li (rd, imm) ->
          Lui (R1, Int32.shift_right_logical imm 16)
          :: Ori (rd, R1, Int32.logand imm (ones 16))
          :: p
      | x -> x :: p)
    prog []
...
let ins2instformat (i : inst) : instformat =
  ...
  | Jal addr -> J { j_opcode = 0x3l; j_addr = addr }
...
let ind2reg (i : int32) : reg = str2reg ("$" ^ Int32.to_string i)

let instformat2ins (f : instformat) : inst =
  match f with
  | R rfor -> (
      match rfor.r_fun with
      | 0x20l -> Add (ind2reg rfor.r_rd, ind2reg rfor.r_rs, ind2reg rfor.r_rt)
      | 0x8l -> Jr (ind2reg rfor.r_rs)
      | _ -> raise FatalError)
  | I ifor -> (
      let regs = ind2reg ifor.i_rs in
...
let inst2str i =
  match i with
  | Add (r1, r2, r3) ->
      "add " ^ reg2str r1 ^ ", " ^ reg2str r2 ^ ", " ^ reg2str r3 ^ "\n"
  | Beq (r1, r2, imm) ->
      "beq " ^ reg2str r1 ^ ", " ^ reg2str r2 ^ ", " ^ Int32.to_string imm
      ^ "\n"
  | Jr r -> "jr " ^ reg2str r ^ "\n"
  ...
  | Ori (r1, r2, imm) ->
      "ori " ^ reg2str r1 ^ ", " ^ reg2str r2 ^ ", " ^ Int32.to_string imm
      ^ "\n"
  | Lw (r1, r2, imm) ->
      "lw " ^ reg2str r1 ^ ", " ^ Int32.to_string imm ^ "(" ^ reg2str r2 ^ ")\n"
  | Sw (r1, r2, imm) ->
      "sw " ^ reg2str r1 ^ ", " ^ Int32.to_string imm ^ "(" ^ reg2str r2 ^ ")\n"

(*get byte 0,1,2,3 of a word*)
let getByte (w : int32) (j : int) : byte =
  mk_byte (Int32.shift_right_logical w (24 - (8 * j)))

let rec assem_code (prog : program) (offset : int32) (init_mem : memory) :
    memory =
  snd
    (List.fold_left
           ...
           mem_update curPos (getByte curWord 0)
             (mem_update (Int32.add curPos 1l) (getByte curWord 1)
                (mem_update (Int32.add curPos 2l) (getByte curWord 2)
                   (mem_update (Int32.add curPos 3l) (getByte curWord 3) curMem)))
         ))
...
let teste i (s, b) =
  ...
  if encoding = s || encoding = b then
    print_string (format_string "passed: " Bright Green ^ prog2str i)
  else
    print_string
      (format_string "failed: " Bright Red
      ^ tostring encoding ^ "   " ^ prog2str i)
