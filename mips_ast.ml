...
let rf_update (r : int) (v : int32) (rf : regfile) : regfile = IntMap.add r v rf
...
let string_of_rf (rf : regfile) : string =
  IntMap.fold
    (fun key v s -> s ^ string_of_int key ^ " -> " ^ Int32.to_string v ^ "\n")
    rf ""
...
(* Basic register type initialization *)
type reg =
  | R0
...
(* NOTE: Basic register to string mapping*)
let reg2str r =
  match r with
  | R0 -> "$0"
  | R1 -> "$1"
...
(* NOTE: Basic string to register mapping *)
let str2reg s =
  match s with
  | "$0" -> R0
  | "$1" -> R1
...
(* NOTE: Basic register to int conversion *)
let reg2ind r =
  match r with
  | R0 -> 0
  | R1 -> 1
...
(* NOTE: Ths declares the constructors of the inst type*)
...
module Int32Map = Map.Make (struct
  type t = int32
...
let mem_update (a : int32) (v : byte) (m : memory) : memory = Int32Map.add a v m
...
let init_regfile =
  reg_of_list [ (28, 0x30000000l); (29, 0x7ffffffcl); (30, 0x7ffffffcl) ]
...
(* NOTE: This is a program to a string *)
let rec prog2str p =
